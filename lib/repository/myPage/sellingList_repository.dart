...
class SellingListRepository {
  User user = Get.find<User>();
  Posts posts = Get.find<Posts>();

  SellingListRepository(); // 생성자

  Future<void> sellingListMethod() async {
    final Dio _dio = Dio();

    try {
      final response = await _dio.post(
        'http://10.0.2.2:8000/myPage/sellingList',
        data: {
          'userId': user.userId,
        },
        options: Options(
      ...
      if (response.statusCode == 200) {
        ...
        posts.sellingListPosts.assignAll(
