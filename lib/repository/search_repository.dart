...
class SearchRepository {
  ...
  SearchRepository(); // 생성자

  Future<void> searchMethod() async {
    final Dio _dio = Dio();
    try {
      final response = await _dio.get(
        'http://10.0.2.2:8000/posts/searchPosts',

        data: {
          'keyword': posts.keyword.value,
          'min': posts.min.value,
          'max': posts.max.value,
          'lineUp': posts.lineUp.value,
          'limit': posts.limit.value,
        },
        options: Options(
          headers: {
